...
async def predict_character(req: PredictRequest):
    if not req.image: return {"status": "error", "message": "Ingen bild mottogs / No image provided."}
    try:
        raw_b64 = req.image.split(",", 1)[1] if "," in req.image else req.image
        img_bytes = base64.b64decode(raw_b64)
        img = Image.open(BytesIO(img_bytes)).convert("L")
    except Exception as e: return {"status": "error", "message": f"Kunde inte tolka bilden: {e}"}
    ...
    if coords.size == 0: return {"status": "error", "message": "Inget tecken hittades / No character detected."}
    ...
    candidates = predict_all_models(np.array(img_28)) if real_model_name in ["Alla modeller", "All models"] else predict_single_model(real_model_name, np.array(img_28))
